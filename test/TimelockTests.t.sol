...
contract TimelockTest is BaseTest {
    ...
    function testStrategy_TimelockProtection() public {
        ...
        assertEq(
            revealModule.getRevealStrategy(), address(inCollectionStrategy), "Should start with in-collection strategy"
        );
        ...
        // Execute operation
        vm.prank(deployer);
        timelock.execute(address(revealModule), 0, data, bytes32(0), revealModule.STRATEGY_UPDATE_OPERATION());
