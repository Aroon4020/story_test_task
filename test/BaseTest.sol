...
contract BaseTest is Test, ERC721Holder {
    ...
    function setUp() public virtual {
        ...
        stakingContract = new StakingContract(address(rewardToken), address(revealModule), deployer);
    ...
    function changeRevealStrategy(address newStrategy) internal {
        ...
        bytes memory data = abi.encodeWithSelector(revealModule.executeStrategyUpdate.selector, newStrategy);

        vm.prank(deployer);
        revealModule.timelock().execute(
            address(revealModule), 0, data, bytes32(0), revealModule.STRATEGY_UPDATE_OPERATION()
        );
